function modelCenter() {
  require([
    ...
    async function displayDirectorates(DirectorateObject) {
      ...
      await query
        .executeQueryJSON(
          "https://192.168.56.56:6443/arcgis/rest/services/MapsDB/MapServer/2",
          {
        ...
        .then((results) => {
          results.features.forEach((element) => {
            Directorates_Repositories.push(element.attributes.DirectorateID);
          });
        });
      for (let i = 0; i < DirectorateObject.length; i++) {
        for (let j = 0; j < Directorates_Repositories.length; j++) {
          if (
            DirectorateObject[i].attributes.DirectorateID ===
            Directorates_Repositories[j]
          ) {
            if (Object.keys(Directorates).length) {
              for (let z = 0; z < Object.keys(Directorates).length; z++) {
                if (Directorates_Repositories[j] !== Directorates[z]) {
    ...
    var RepositoriesLayer = new FeatureLayer({
    ...
    map.add(RepositoriesLayer); // adds the layer to the map
    /*****************************************************************/
    ...
    view.on("layerview-create", (event) => {
      ...
      if (event.layer.id === "Repositories") {
     ...
     view.on("click", (event) => {
      ...
      view
        ...
        .then((objectId) => {
          ...
          return  RepositoriesLayer
            .queryRelatedFeatures({
              outFields: ["*"],
              relationshipId:  RepositoriesLayer.relationships[2].id,
              objectIds: objectId,
            })
            .then((results) => {
              // console.log(results);
            ...
            .then(function () {
              return  RepositoriesLayer
                .queryRelatedFeatures({
                  outFields: ["*"],
                  relationshipId:  RepositoriesLayer.relationships[4].id,
                  objectIds: objectId,
                })
                .then((results) => {
                  results[objectId].features.forEach((element) => {
            ...
            .then(() => {
              return  RepositoriesLayer
                .queryRelatedFeatures({
                  outFields: ["*"],
                  relationshipId:  RepositoriesLayer.relationships[1].id,
                  objectIds: objectId,
                })
                .then((results) => {
                  results[objectId].features.forEach((element) => {
                          ...
                          return query
                            ...
                            .then((oid) => {
                              ...
                              return query
                                ...
                                .then((oid) => {
                                  console.log(
                                    oid.features[0].attributes["Product_Vegetarian_Name"]
                                  ); //2
                                  Product = oid.features[0].attributes["Product_Vegetarian_Name"];
        ...
        .then(() => {
          RepositoriesLayer.popupTemplate = {
